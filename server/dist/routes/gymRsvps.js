...
router.get('/', requireAdmin, async (_req, res) => {
    try {
        const items = await getAll('gym-rsvps');
        res.json(items);
    }
    catch {
...
router.post('/', async (req, res) => {
    try {
        ...
        const sessions = await getAll('gym-live-sessions');
        const session = sessions.find(s => s.id === sessionId && s.status !== 'cancelled');
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
        }
        const existing = await getAll('gym-rsvps');
        const duplicate = existing.find(r => r.sessionId === sessionId && String(r.email || '').toLowerCase() === String(email).toLowerCase());
        if (duplicate) {
...
//# sourceMappingURL=gymRsvps.js.map