...
export function createEmployeesRouter() {
  ...
  router.get('/', requireAdmin, async (_req, res) => {
    try {
      const items = await getAll<Record<string, unknown>>('employees')
  ...
  router.get('/me', requireEmployee, async (req, res) => {
    try {
      const employee = await getById<any>('employees', req.user!.employeeId!)
      if (!employee) { res.status(404).json({ error: 'Employee not found' }); return }
      const { passwordHash, ...safe } = employee
      const attendance = await getAll<any>('attendance')
      const leaves = await getAll<any>('leave-requests')
      res.json({
        ...safe,
  ...
  router.get('/:id', requireAdmin, async (req, res) => {
    try {
      const item = await getById<any>('employees', req.params.id)
  ...
  router.post('/', requireAdmin, async (req, res) => {
    try {
      ...
      const body: Record<string, unknown> = { ...rest, active: rest.active ?? true }
  ...
  router.get('/:id/attendance', requireAdmin, async (req, res) => {
    try {
      const all = await getAll<any>('attendance')
  ...
  router.post('/:id/attendance', requireEmployee, async (req, res) => {
    try {
      ...
      const action = (req.body.action as string) || 'checkin'
      const all = await getAll<any>('attendance')
...
function stripPassword(item: Record<string, unknown>) {
  if (!item) return item
  const { passwordHash, ...rest } = item as Record<string, unknown> & { passwordHash?: string }
  return rest
}
