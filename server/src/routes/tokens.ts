...
async function findByDate(date: string): Promise<DailyToken[]> {
  const all = await getAll<DailyToken & Record<string, unknown>>('tokens')
 ...
 */
export async function findValidToken(date: string, value: unknown): Promise<DailyToken | null> {
  if (!date || typeof value !== 'string') return null
  const normalized = value.trim().toUpperCase()
  const all = await getAll<any>('tokens')
...
export function createTokensRouter() {
  ...
  router.get('/', requireAdmin, async (_req, res) => {
    try {
      const items = await getAll<DailyToken>('tokens')
  ...
  router.post('/', requireAdmin, async (req, res) => {
    try {
      ...
      const token = (req.body.token as string) || generateToken()
